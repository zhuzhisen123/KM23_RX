...
{
#if (defined DEV_UPDATE_SUPPORT_JUMP) || (defined UPDATE_SUPPORT_JUMP_BT_UPDATA)
    ...
    extern void __BT_UPDATA_JUMP(u32 spi_port);
    u32 spi_port = 0;
#if BOOT_SPI_GET_PORT_FUNC
    spi_port = spi_get_port() ? BOOT_SPI_NORFLASH_PORTB : BOOT_SPI_NORFLASH_PORTA;
#endif
    log_i("spi_port:0x%x\n", spi_port);
    y_printf("update jump to __BT_UPDATA ...\n");
    y_printf(">>>[test]:JL_IOMC = 0x%x\n", (JL_IOMAP->CON0 >> 16) & 0x3);
    //跳转到uboot加载完,30ms左右(200410_yzb)
    __BT_UPDATA_JUMP(spi_port);
#else
