...
void usb_start()
{
    ...
    if(get_mic_spk_sta()){
        ...
        usb_device_mode(usbfd, USB_DEVICE_CLASS_CONFIG_USR);
    }else{
    ...
    if(app_var.spk_timeout_timer){
        sys_timeout_del(app_var.spk_timeout_timer);
        app_var.spk_timeout_timer = 0;
    }
