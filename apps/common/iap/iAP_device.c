...
extern u32 MCU_SRAMToUSB_app(void *hdl, u8 *pBuf, u16 uCount);
extern u32 MCU_USBToSRAM_app(void *hdl, u8 *pBuf, u16 uCount);
...
static u32 iAP_bulk_in_process(void)
{
    u32 wlen, len;
...
static u32 iAP_bulk_out_process(void)
{
    u32 len;
...
u8 apple_mfi_link(void *hdl)
{
    ...
    if (IAP2_VERSIONS == iAP_support()) {
		...
		memcpy(&iAP2_message_name[20+USER1_LEN+2], user_id, 32);
        /* iAP_deg_str("iap2"); */
        /* os_time_dly(2); */
		...
		#if 1 // DC_CHECK_ENABLE
			iAP2_power_to_apple_PowerStart();//
			// if(gpio_read(IO_PORTB_04)){
...
extern volatile u8 app_suport_redo; //不支持重发 - 用于过认证（也可以量产）app_suport_redo = 1;//支持重发 - 建议量产时支持重发
