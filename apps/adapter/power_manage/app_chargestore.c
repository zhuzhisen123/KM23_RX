...
#define CMD_GET_RD32                 0x82    //获取一个随机数
#define CMD_SET_CHECK_MIC_PAIR_CODE  0x83    //设置及检查麦配对吗
#define CMD_LOWPOWER_SHUT_DOWN       0x84    //低电下的关机，会让耳机关掉出仓唤醒
...
void chargestore_send_msg_shutdown(void)
{
    struct sys_event e;
    e.type = SYS_DEVICE_EVENT;
    e.arg  = (void *)DEVICE_EVENT_FROM_CHARGE;
    e.u.dev.event = CHARGE_EVENT_SHUT_DOWN;
    e.u.dev.value = NULL;
    sys_event_notify(&e);
}
...
void chargestore_shutdown_do(void *priv)
{
    ...
    if (p) {
        log_info("send chargestore lowpower shutdown!\n");
        e.u.dev.event = CHARGE_EVENT_LOWPOWER_SHUT_DOWN;
    } else {
        log_info("send chargestore shutdown!\n");
        e.u.dev.event = CHARGE_EVENT_SHUT_DOWN;
    }
    e.type = SYS_DEVICE_EVENT;
    e.arg  = (void *)DEVICE_EVENT_FROM_CHARGE;
...
void app_chargestore_data_deal(u8 *buf, u8 len)
{
    ...
    case CMD_SHUT_DOWN:
        ...
        break;
#if TCFG_CHARGESTORE_ENABLE
    case CMD_LOWPOWER_SHUT_DOWN:
        log_info("lower shut down\n");
        __this->power_status = 0;
        __this->cover_status = 0;
        __this->close_ing = 0;
        chargestore_api_write(send_buf, 1);
        __this->shutdown_timer = sys_hi_timer_add(1, chargestore_shutdown_do, 1000);
        break;
#endif
    case CMD_CLOSE_CID:
        log_info("close cid\n");
