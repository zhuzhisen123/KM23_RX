...
static u16 sof_frame = 0, sof_frame_last = 0;
static int usb_sof_detect_timer = 0;
static void usb_sof_detect()
{
    putchar('f');
    sof_frame_last = sof_frame;
    sof_frame = usb_read_sofframe(0);

    if (sof_frame != sof_frame_last) {
        putchar('1');
        if(usr_usb_online==0){
            usr_usb_online = 1;
        }
    } else {
        printf("Y----------usb offline----------N\n");
        sys_timer_del(usb_sof_detect_timer);
        usb_sof_detect_timer = 0;
        struct usb_device_t *usb_device = usb_id2device(0);
...
void usb_isr(const usb_dev usb_id)
{
    ...
    if (intr_tx & BIT(0)) {
        if (usb_interrupt_rx[usb_id][0]) {
            ...
            if(usb_sof_detect_timer==0){
                usb_sof_detect_timer = sys_timer_add(0, usb_sof_detect, 3000);
            }
        }
    }
