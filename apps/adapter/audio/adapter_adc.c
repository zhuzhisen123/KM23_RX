...
int adapter_adc_mic_open(u16 sample_rate, u8 gain, u16 irq_points)
{
...
#if TCFG_EQ_ENABLE
    if (eq_online_get_flag()) {
        audio_adc_linein_set_gain(&hdl->mic_ch, eq_online_get_linein_gain());
        audio_adc_linein1_set_gain(&hdl->mic_ch, eq_online_get_linein_gain());
        if (!eq_online_get_gain_boost()) {
...
#endif
    {
        ...
        audio_adc_linein_0dB_en(1);//linein默认关掉前级6dB
        audio_adc_linein1_set_gain(&hdl->mic_ch, gain);
        audio_adc_linein1_0dB_en(1);//linein默认关掉前级6dB
    }
...
#if TCFG_EQ_ENABLE
    if (eq_online_get_flag()) {
        audio_adc_mic_set_gain(&hdl->mic_ch, eq_online_get_mic_gain());
        if (!eq_online_get_gain_boost()) {
#if CONFIG_CPU_BR28
...
void adapter_adc_gain_set(u8 mic_gain, u8 linein_gain)
{
    if (adapter_adc_mic) {
#if WIRELESS_MIC_STEREO_EN
        audio_adc_linein_set_gain(&adapter_adc_mic->mic_ch, linein_gain);
#if CONFIG_CPU_BR28
        audio_adc_linein2_set_gain(&adapter_adc_mic->mic_ch, linein_gain);
#elif CONFIG_CPU_BR30
        audio_adc_linein1_set_gain(&adapter_adc_mic->mic_ch, linein_gain);
#endif
#else
        audio_adc_mic_set_gain(&adapter_adc_mic->mic_ch, mic_gain);
#endif
    }
}
...
void adapter_adc_6dB_en(u8 en)
{
    ...
    if (adapter_adc_mic) {
#if WIRELESS_MIC_STEREO_EN
        audio_adc_linein_0dB_en(en);
#if CONFIG_CPU_BR28
        audio_adc_linein2_0dB_en(en);
#elif CONFIG_CPU_BR30
        audio_adc_linein1_0dB_en(en);
#endif
#else
#if CONFIG_CPU_BR28
        audio_adc_mic_0dB_en(en);
#elif CONFIG_CPU_BR30
        audio_mic_0dB_en(en);
#endif
#endif
...
{
    adapter_adc_gain_set(mic_gain, line_gain);
    adapter_adc_6dB_en(boost);
}
