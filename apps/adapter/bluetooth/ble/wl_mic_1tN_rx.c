...
#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_WL_MIC_1TN_RX)

#if LE_DEBUG_PRINT_EN
#define log_info(x, ...)    printf("[wlm_1tN_rx]" x " ", ## __VA_ARGS__)
...
#define WL_MIC_SPECIFIC_NAME "TEST_VER"
...
/* 内部函数声明 */
static void wlm_1tN_rx_connect_succ_callback(void);
static void wlm_1tN_rx_disconnect_callback(void);
static void wlm_1tN_rx_iso_callback(const void *const buf, size_t length);
static void wlm_1tN_rx_set_pair_trigger(uint32_t param, uint32_t type);
void wlm_1tN_rx_set_pair(uint32_t param);
...
static wlm_param rx_param = {
    ...
    .param = {
        ...
        [1] = 28,
#endif
        [2] = TCFG_WIRELESS_RSSI,
        [3] = RX_ADAPTIVE_PARAM,
        [7] = 1,
    },
};

static const wlm_lib_callback wlm_1tN_rx_cb = {
    .wlm_connect_succ     = wlm_1tN_rx_connect_succ_callback,
    .wlm_disconnect       = wlm_1tN_rx_disconnect_callback,
    .wlm_iso_rx           = wlm_1tN_rx_iso_callback,
    .wlm_set_pair_trigger = wlm_1tN_rx_set_pair_trigger,
};

static const wlm_lib_ops *const wlm_op = &wlm_1tN_rx_op;
...
{
    log_info("wl_mic_1tN_rx profile init");
    ...
    if (__this && __this->wlm_init) {
        ...
        log_info("bt_name = %s", rx_param.pair_name);
#endif
        __this->wlm_init(&rx_param, &wlm_1tN_rx_cb);
    }
}
...
#endif /* (TCFG_BLE_DEMO_SELECT == DEF_BLE_WL_MIC_1TN_RX) */
