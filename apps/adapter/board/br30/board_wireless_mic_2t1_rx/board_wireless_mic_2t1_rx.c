...
#elif TCFG_SHARE_OSC_EN
struct port_wakeup ldoin_fall_port = {
	.edge               = FALLING_EDGE,                      //唤醒方式选择,可选：上升沿\下降沿
	.both_edge          = 0,
	.filter             = PORT_FLT_16ms,
	.iomap              = TCFG_SHARE_OSC_UART_PORT,                      //唤醒口选择
};
#endif
...
#if ADAPTER_UART_DEMO
	portb_value &= ~BIT(2);//默认使用PB2作为通信口，如果IO口不是PB2，请对应修改!!!
#endif
	/*用于四线flash*/
...
#if TCFG_SHARE_OSC_EN
	if (get_reset_source_value() & (BIT(16)|BIT(17))) {
		log_info("vddio por,soudbox and wlm_rx enter soft off with soundbox ENABLE TCFG_POWER_ON_NEED_KEY");
		//音箱是软开关机方案，插入电池音箱会关机，RX也需要关机。如果音箱是硬开关的，需要将下面关机函数注释掉，让RX直接开机
		power_set_soft_poweroff();
	} else {
		log_info("no vddio por");
	}
	printf("disable ldoin wakeup");
