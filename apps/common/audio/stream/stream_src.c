...
static int stream_src_run(struct __stream_src *stream, s16 *data, int len)
{
    ...
    int wlen = audio_src_resample_write(stream->src_hdl, data, len);
    if (wlen < len) {
        audio_resample_hw_trigger_interrupt(stream->src_hdl, (void *)&stream->entry, (void (*)(void *))audio_stream_resume);
    }
    return wlen;
}
