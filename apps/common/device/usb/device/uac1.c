...
static u32 spk_transfer_len  = 0xffff;
...
extern u8 uac_speaker_stream_status(void);
static void sof_isr(struct usb_device_t *usb_device, u32 ep)
{
    ...
    if (spk_transfer_len == 0) {
        ...
        uac_speaker_stream_write(buffer, sizeof(buffer));
    }
    spk_transfer_len = 0;
}
...
static void open_spk(struct usb_device_t *usb_device)
{
    ...
    usb_g_set_intr_hander(usb_id, 0 | USB_DIR_IN, sof_isr);
#endif
...
#endif
#if USB_EP_PROTECT
        u32 ep_reserved_len = 1024 - SPK_FRAME_LEN;
#else
        u32 ep_reserved_len = 0;
#endif
        if (class == AUDIO_CLASS) {
            uac_info->spk_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, SPK_ISO_EP_OUT, SPK_FRAME_LEN + MIC_FRAME_LEN + ep_reserved_len);
            uac_info->mic_dma_buffer = uac_info->spk_dma_buffer + SPK_FRAME_LEN + ep_reserved_len;
        } else if (class == SPEAKER_CLASS) {
            uac_info->spk_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, SPK_ISO_EP_OUT, SPK_FRAME_LEN + ep_reserved_len);
