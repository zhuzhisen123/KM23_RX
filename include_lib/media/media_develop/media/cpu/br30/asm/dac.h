...
struct audio_dac_hdl {
    ...
    /*******************************************/
    u8 need_resume;                 /*DAC 是否需要唤醒的变量*/
    void (*resume_callback)(void);  /* 唤醒DAC 的回调函数*/
};
...
void audio_dac_channel_set_underrun_handler(struct audio_dac_channel *ch, void *priv, void (*handler)(void *, int));
...
/*************************************************************************
 * 设置唤醒数据流的回调函数
 * INPUT    :  dac - DAC设备句柄，resume_callback - 设置进去的回调函数
 * OUTPUT   :  无
 * WARNINGS :  无.
 *=======================================================================*/
void  audio_dac_set_resume_callback(struct audio_dac_hdl *dac, void (*resume_callback)(void));
