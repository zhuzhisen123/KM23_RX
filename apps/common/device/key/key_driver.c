...
#if 1
#define LED_INIT_EN()     	    do{gpio_set_pull_up(IO_PORTB_07, 0);gpio_set_pull_down(IO_PORTB_07, 0);gpio_set_direction(IO_PORTB_07, 1);\
									gpio_set_pull_up(IO_PORTB_08, 0);gpio_set_pull_down(IO_PORTB_08, 0);gpio_set_direction(IO_PORTB_08, 1);\
									}while(0)

void wlm_rx_led_deinit()
{
...
void spk_led_deal()
{
    if(!(app_var.spk_switch_cnt%2)){ // 6 4 2
        wlm_rx_led(1);
        wlm_denoise_led(0);
    }else{ // 5 3 1
        wlm_rx_led(0);
    ...
    app_var.spk_switch_cnt--;
    if(app_var.spk_switch_cnt==0){
        sys_timer_del(app_var.spk_led_timer);
        app_var.spk_led_timer = 0;
        wlm_rx_led(0);
        wlm_denoise_led(0);
    }
}

void denoise_led_deal()
{
    if(!(app_var.denoise_switch_cnt%2)){ // 6 4 2
        wlm_rx_led(!app_var.is_denoise_led);
        wlm_denoise_led(app_var.is_denoise_led);
    }else{ // 5 3 1
        wlm_rx_led(0);
        wlm_denoise_led(0);
    }

    app_var.denoise_switch_cnt--;
    if(app_var.denoise_switch_cnt==0){
        sys_timer_del(app_var.denoise_led_timer);
        app_var.denoise_led_timer = 0;
        wlm_rx_led(0);
        wlm_denoise_led(0);
    }
}
...
void led_scan(void)
{
    ...
    static u8 mono_stereo_flag = 0xff;
    ...
    static u8 flag_250ms = 0;
    static u8 flag_500ms = 0;
    static u8 flag_1s = 0;
    static u8 low_pwr_cnt = 0;
...
{
    static u8 phone_cnt_online;
    static u8 phone_cnt_outline;

    gpio_set_pull_down(IO_PORTB_03, 1);
	gpio_set_pull_up(IO_PORTB_03, 0);//gpio_set_pull_up(IO_PORTB_08, 1);
	gpio_set_direction(IO_PORTB_03, 1);
	gpio_set_die(IO_PORTB_03, 1);
    if(gpio_read(IO_PORTB_03)==0){
        phone_cnt_online = 0;
        phone_cnt_outline++;
        if(phone_cnt_outline > 2){
            phone_cnt_outline = 0;
			if(mfi_pass_status == MFI_PROCESS_PASS){
	            if(dc_online){
	                dc_online = 0;
                    //puts("000000000000000000000000000A");
					iAP2_power_from_apple_PowerUpdate(0);//充电电流配置
	            }
			}
        }
    }else{
        ...
        if (phone_cnt_online > 2){
            phone_cnt_online = 0;
			if(mfi_pass_status == MFI_PROCESS_PASS){
                dc_online = 1;
                //puts("333333333333333333333333333A");
                iAP2_power_from_apple_PowerUpdate(3000);//充电电流配置
			}
        }
    }
}
...
static void key_driver_scan(void *_scan_para)
{
    ...
    static u8 pwr_on_delay = 100;
    pwr_on_delay--;
    if(key_poweron_flag && (!pwr_on_delay)){
        key_poweron_flag = 0;
    }
