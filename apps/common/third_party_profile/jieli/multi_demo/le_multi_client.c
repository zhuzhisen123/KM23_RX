...
#if TCFG_2T1_RX_PRODUCT_TEST_EN
#define TEST_MIC_BLE_NAME	  "MIC_TEST_2T1"
            if (0 == memcmp(adv_data_pt, TEST_MIC_BLE_NAME, sizeof(TEST_MIC_BLE_NAME) - 1)) {
                log_info("match MIC TEST name\n");
                bt_ble_scan_enable(0, 0);
                client_event_report(CLI_EVENT_ENTER_TEST_AUDIO_SYNC, NULL, 0);
                //find_remoter = 1;
                break;
            }
#endif
...
static u8 client_set_bond_info(u8 *addr, u8 addr_type, u8 event_type)
{
    u8 find_tag = 0;
    u8 pair_cnt = 0;
    int i;
    ...
    for (i = 0; i < SUPPORT_MAX_CLIENT; i++) {
        if (conn_pair_info_table[i].pair_flag) {
            pair_cnt ++;
            //printf("addr_type=%d,conn_pair_info_table[i].peer_address_info[0]=%d",addr_type,conn_pair_info_table[i].peer_address_info[0]);
            //put_buf(addr,6);
            //put_buf(&conn_pair_info_table[i].peer_address_info[1],6);
            if (addr_type == conn_pair_info_table[i].peer_address_info[0]
                && 0 == memcmp(addr, &conn_pair_info_table[i].peer_address_info[1], 6)) {
#if WIRELESS_PAIR_BONDING
                if ((event_type != 1) && (event_type != 0x15)) {
                    return 0xff;
                }
#endif
...
static void client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    find_tag = client_set_bond_info(report_pt->address, report_pt->address_type, report_pt->event_type);

    //printf("find_tag=%d",find_tag);
    if (find_tag == 0xff) {
        return ;
    }
    if (!find_tag) {
...
static void client_report_ext_adv_data(u8 *report_pt, u16 len)
{
    ...
    u8 event_type = report_pt[GET_STRUCT_MEMBER_OFFSET(__ext_adv_report_event, Event_Type)];
    s8 rssi = report_pt[GET_STRUCT_MEMBER_OFFSET(__ext_adv_report_event, RSSI)];
    find_tag = client_set_bond_info(address, address_type, event_type);
    ...
    if (find_tag) {
        if (force_seach_onoff && force_seach_rssi > rssi) {
...
static void client_create_connection(u8 *conn_addr, u8 addr_type)
{
    ...
    if (pair_bond_enable == 0x55) {
        ...
        if (config_vendor_le_bb_slots) {
            create_conn_par->conn_interval = config_vendor_le_bb_slots;
        } else {
...
static void bt_ble_create_connection(u8 *conn_addr, u8 addr_type)
{
#if EXT_ADV_MODE_EN && (0 == TCFG_2T1_RX_PRODUCT_TEST_EN)
...
static int bt_ble_scan_enable(void *priv, u32 en)
{
    ...
    if (en) {
        //第二台扫描参数由底层决定
#if 0//(APP_MAIN == APP_WIRELESS_DUALMIC)
...
#if (WIRELESS_24G_ENABLE)
    rf_set_24g_hackable_coded(WIRELESS_24G_CODE_ID);
    extern void rf_set_ext_24g_hackable_code(uint32_t code);
    rf_set_ext_24g_hackable_code(WIRELESS_24G_CODE_ID);
#endif
#endif
