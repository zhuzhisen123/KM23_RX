...
void ci_data_rx_handler(u8 type)
{
    ...
    if (type == CI_UART && __this->udev) {
        __this->data_length += __this->udev->read(&__this->pRxBuffer[__this->data_length], (UART_RX_SIZE - __this->data_length), 10);//串口读取buf剩余空间的长度，实际长度比buf长，导致越界改写问题
    }
    ...
    if (procotol) {
        ...
        if (__this->data_length >= p_newtool->length + head_data_num) {
            ...
            /* printf("uart rx-----------------\n"); */
            /* log_info_hexdump(p_newtool, p_newtool->length + head_data_num); */
            online_cfg_tool_data_deal(p_newtool, p_newtool->length + head_data_num);
        } else {
            return;
        }
...
static int ci_uart_init()
{
    ...
    extern void cfg_tool_share_osc_check_single_io_send_init();
    cfg_tool_share_osc_check_single_io_send_init();
#endif
    return 0;
}
