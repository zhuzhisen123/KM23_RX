...
static int wlm_hid_send_timer = 0;
void wlm_hid_send()
{
    printf("%s:%d\n", __func__, __LINE__);
    custom_hid_tx_data(0, g_hid_send_buf, 64);

    sys_timeout_del(wlm_hid_send_timer);
    wlm_hid_send_timer = 0;
}
#include "syscfg_id.h"
#include "key_event_deal.h"
extern int app_task_put_key_msg(int msg, int value);
static void custom_hid_rx_data(struct usb_device_t *usb_device, u32 ep)
{
    ...
    y_printf("%s:%d\n", __func__, usb_id);
    put_buf(rx_buffer, 64);
    memcpy(g_hid_send_buf, rx_buffer, rx_len);
    if(wlm_hid_send_timer==0){
        wlm_hid_send_timer = sys_timeout_add(NULL, wlm_hid_send, 10);
    }
    if (custom_hid_info && custom_hid_info->hid_rx_hook) {
        //put_buf(rx_buffer,rx_len);
        //custom_hid_tx_data(usb_id,rx_buffer,rx_len);
