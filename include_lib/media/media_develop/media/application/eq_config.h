...
enum {
    ...
    EQ_ONLINE_CMD_DEV_GAIN = 0x2202,//

    EQ_ONLINE_CMD_NOISEGATE = 0x2204,
    EQ_ONLINE_CMD_FREQ_SHIFT_HOWLING = 0x2205,
    EQ_ONLINE_CMD_NOTCH_HOWLING = 0x2206,
    EQ_ONLINE_CMD_VOICE_CHANGER = 0x2207,
...
#define AEID_MIC_HOWLING_PS       0xa
#define AEID_MIC_HOWLING_SUPRESS  0xb
#define AEID_MIC_NS_GATE          0xc
#define AEID_MIC_VOICE_CHANGER    0xd
/*eq online packet*/
typedef struct {
...
struct dev_gain {
    EQ_FILE_HEAD head;//头结构
    int mic_gain_boost;// 0或者1
    int mic_gain;  //0~19
    int dac_gain;//br30 0~15,  br28 0~3
};

struct noisegate_par {
    EQ_FILE_HEAD head;//头结构
    int is_bypass;
    noisegate_update_param parm;
};

struct freq_shift_howling_par {
    EQ_FILE_HEAD head;//头结构
    int is_bypass;
    HOWLING_PITCHSHIFT_PARM parm;
};

struct notch_howling_par {
    EQ_FILE_HEAD head;//头结构
    int is_bypass;
    NotchHowlingParam parm;
};
struct voice_changer_par {
    EQ_FILE_HEAD head;//头结构
    int is_bypass;          // 1-> byass 0 -> no bypass
    int mode;
    VOICECHANGER_PARM parm[10];
};
...
typedef struct {
    ...
    struct dev_gain gain;
    struct noisegate_par ns_parm;
    struct freq_shift_howling_par ps_howling_parm;
    struct notch_howling_par notch_howling_parm;
    struct voice_changer_par voice_changer_parm;
    u8 cur_mode;
