...
/*const */u8 iAP2_message_name[] = {
    ...
    //MessagesSentByAccessory：声明可以发送的命令
    0x00, 4 + 3 * 2, 0x00, 0x06,
    ...
    //MessagesReceivedFromDevice：声明可以接受的命令
    0x00, 4 + 1 * 2, 0x00, 0x07, (u8)(POWER_UPDATES >> 8), (u8)(POWER_UPDATES),

    //PowerSourceType：2 - Self powered(自供电)
    0x00, 4 + 1, 0x00, 0x08, 0x02,

#if IS_WITH_BATTERY
    //MaximumCurrentDrawnFromDevice. if PowerSourceType is 2, set 0; else can set 0, 500, 1000
    0x00, 4 + 2, 0x00, 0x09, 0x00, 0x00,
#else
    //MaximumCurrentDrawnFromDevice. if PowerSourceType is 2, set 0; else can set 0, 500, 1000
    0x00, 4 + 2, 0x00, 0x09, 0x00, 0x64,
#endif

    //CurrentLanguage--can not alter
    0x00, 4 + IAP2_CUR_LANGUAGE_LEN, 0x00, 0x0C, IAP2_CUR_LANGUAGE,

    //SupportedLanguage--can not alter
    0x00, 4 + IAP2_SUP_LANGUAGE_LEN, 0x00, 0x0D, IAP2_SUP_LANGUAGE,

    //USBHostTransportComponent--can alter
    0x00, 4 + (4 + 2) + (4 + IAP2_TRANSPORT_NAME_LEN) + (4), 0x00, 0x10,
    0x00, 4 + 2, 0x00, 0x00, 0x00, 0x01, //TransportComponentIdentifier
    0x00, 4 + IAP2_TRANSPORT_NAME_LEN, 0x00, 0x01, IAP2_TRANSPORT_NAME, //TransportComponentName：
    0x00, 4, 0x00, 0x02,  //TransportSupportsiAP2Connection
};
