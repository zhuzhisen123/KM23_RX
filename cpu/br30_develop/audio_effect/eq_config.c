...
eq_online_gain var_gain = {
    .mic_gain_boost = 1,
    .mic_gain = WIRELESS_MIC_ADC_GAIN,
    .dac_gain = 15,
    .eq_online_flag = 0,
};
...
eq_tool_cfg eq_tool_tab[] = {
    ...
    {wirless_mic_mode, (u8 *)"麦克风音效", 	0x3100, SONG_SECTION, 6, {EQ_ONLINE_CMD_DEV_GAIN,  EQ_ONLINE_CMD_NOISEGATE, EQ_ONLINE_CMD_FREQ_SHIFT_HOWLING, EQ_ONLINE_CMD_NOTCH_HOWLING, EQ_ONLINE_CMD_VOICE_CHANGER, EQ_ONLINE_CMD_SONG_DRC}},
};
 ...
 *获取mic与dac增益，例子
 * */
static u8 init_return = 0;
void get_audio_dev_gain()
{
    /* if (!init_return) { */
    /*     init_return = 1; */
    /*     return; */
    ...
    if (eq_cfg) {
        printf("mic gain_boost %d, mic gain %d, dac gain %d\n",
               eq_cfg->cfg_parm[wirless_mic_mode].gain.mic_gain_boost,
               eq_cfg->cfg_parm[wirless_mic_mode].gain.mic_gain,
               eq_cfg->cfg_parm[wirless_mic_mode].gain.dac_gain);
        var_gain.mic_gain_boost = eq_cfg->cfg_parm[wirless_mic_mode].gain.mic_gain_boost;
        var_gain.mic_gain = eq_cfg->cfg_parm[wirless_mic_mode].gain.mic_gain;
        var_gain.dac_gain = eq_cfg->cfg_parm[wirless_mic_mode].gain.dac_gain;
        var_gain.eq_online_flag = 1;
        adapter_adc_set(var_gain.mic_gain, var_gain.mic_gain, var_gain.mic_gain_boost);
    }
}
u8 eq_online_get_mic_gain()
{
    return var_gain.mic_gain;
}
...
{
    return var_gain.mic_gain_boost;
}
u8 eq_online_get_dac_gain()
{
    return var_gain.dac_gain;
}
u8 eq_online_get_flag()
{
    return var_gain.eq_online_flag;
}
